 ...
 * to `max_nesting` deep and return the resovled direct reference. If any of
 * the intermediate symbolic references points to a non-existing reference,
 * then that symbolic reference is returned instead with an error code of
 * `GIT_ENOTFOUND`. If the given reference is a direct reference already, it is
 * returned directly.
 *
 ...
 * @return `0` on success, a negative error code otherwise. If `GIT_ENOTFOUND`
 *         is returned, the `out` pointer may be allocated to the last symbolic
 *         reference that's pointing to a non-existent target reference.
 */
